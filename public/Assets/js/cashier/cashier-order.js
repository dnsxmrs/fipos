...
function addItemToOrder(button) {
    ...
    if (orderItems[name]) {
        ...
        const orderItem = `
                    <div class="flex items-center justify-evenly mt-2 text-sm space-x-3" id="item-${name}">
                        <span class="w-1/4 text-left">
                            <p class="text-base">${name}</p>
                            <p class="text-xs text-gray-500 mt-1">₱${price.toFixed(2)}</p>
                        </span>
                        <span class="w-1/4 text-center flex items-center justify-center space-x-2">
                            <button class="decrease-qty p-3 flex items-center justify-center border border-gray-500 hover:bg-gray-200 bg-white text-gray-500 text-sm font-semibold rounded-md w-[20px] h-[20px]" onclick="changeQuantity('${name}', -1)">-</button>
                            <span class="item-qty">${
                                orderItems[name].quantity
                            }</span>
                            <button class="increase-qty p-3 flex items-center justify-center border border-gray-500 hover:bg-gray-200 bg-white text-gray-500 text-sm font-semibold rounded-md w-[20px] h-[20px]" onclick="changeQuantity('${name}', 1)">+</button>
                        </span>
                        <span class="w-1/4 text-center item-price">₱ ${orderItems[
                            name
                        ].price.toFixed(2)}</span>
                        <button class="remove-item text-red-700 p-1 cursor-pointer hover:bg-red-500 hover:bg-opacity-20 rounded-full text-center" onclick="removeItem('${name}')"><i class="fa-solid fa-ban w-13"></i></button>
                    </div>
                `;
