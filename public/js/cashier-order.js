    ...
    if (orderItems[name]) {
        ...
        const orderItem = `
                    <div class="flex justify-between mt-2 text-sm ml-6" id="item-${name}">
                        ...
                        <span class="w-1/3 text-center item-price">₱ ${orderItems[
                            name
...
{
    total += priceChange; // Update the global total variable
    ...
    // Update the subtotal and payable amount
    subTotal = total - discountAmount; // Update global subTotal
    payableAmount = subTotal + taxAmount; // Update global payableAmount
